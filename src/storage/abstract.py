...
from storage import firebase_manager
...
class FirebaseStorage(StorageManager):
    ...
    def save_user(self, user: User):
        firebase_manager.save_user(user)

    def load_user(self, user_id: int) -> User | None:
        return firebase_manager.load_user(user_id)

    def update_user_fields(self, user_id: int, fields: dict) -> None:
        firebase_manager.update_user_fields(user_id, fields)

    def load_user_fields(
        self, user_id: int, fields: set[str] | None = None
    ) -> dict | None:
        return firebase_manager.load_user_fields(user_id, fields)
